...
class PipelineCompiler:
    ...
    def run(self) -> BuildResult:
        ...
        # 14b. Copy custom asset overrides (like post_effects, shaders, custom sun/sky)
        custom_assets_dir = self.paths.source / "assets"
        if custom_assets_dir.is_dir():
            self.logger.step("Copying custom asset overrides")
            self._copy_custom_assets(custom_assets_dir, self.paths.generated / "assets")
    ...
    def _copy_custom_assets(self, src: Path, dest: Path) -> None:
        import shutil
        copied_count = 0
        for item in src.rglob("*"):
            if item.is_file():
                ...
                copied_count += 1
        self.logger.info(f"Copied {copied_count} custom asset file(s) from source/assets")
