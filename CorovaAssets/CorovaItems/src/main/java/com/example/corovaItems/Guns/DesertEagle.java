...
/**
 * Desert Eagle — High-Power Pistol
 * Damage: 22 | Mag: 7 | Reload: 2.0s | Range: 55
 * Right-click: Fire | Shift Drop: Reload
 */
public class DesertEagle extends CorovaItems implements Listener {
    ...
    public DesertEagle() {
        super("§6Desert Eagle", Material.GOLDEN_HOE,
                List.of("§7Heavy Semi-Auto Pistol",
                        "§7Damage: §c50 §7| Mag: §e6 §7| Reload: §a2.0s",
                        "§8Right-click: Fire  |  Shift Drop: Reload"),
                Collections.emptyMap(), "desert_eagle");
    }

    private JavaPlugin pl()               { return JavaPlugin.getProvidingPlugin(DesertEagle.class); }
    ...
    private void reload(Player p, ItemStack stack) {
        ...
        p.sendMessage("§eReloading... §7(2.0s)");
    ...
    private void doShot(Player p, ItemStack stack) {
        ...
        for (double i = 0.5; i < RANGE; i += 0.3) {
            ...
            p.getWorld().spawnParticle(Particle.ENCHANTED_HIT, loc, 1, 0, 0, 0, 0);
            for (Entity e : p.getWorld().getNearbyEntities(loc, HIT_RADIUS, HIT_RADIUS, HIT_RADIUS)) {
                if (e instanceof LivingEntity t && e != p) {
                    ...
                    loc.getWorld().spawnParticle(Particle.EXPLOSION, loc, 3, 0.1, 0.1, 0.1, 0);
                    loc.getWorld().spawnParticle(Particle.ENCHANTED_HIT, loc, 12, 0.2, 0.2, 0.2, 0.05);
                    hit = true; break;
                }
            }
            if (hit) break;
        ...
        p.getWorld().playSound(p.getLocation(), Sound.ENTITY_GENERIC_EXPLODE, 1f, 1.6f); bar(p, stack);
