...
/**
 * PP2000 — Machine Pistol / SMG
 * High fire rate, high spread, compact.
 */
public class PP2000 extends CorovaItems implements Listener {
    ...
    public PP2000() {
        super("§7PP2000", Material.IRON_HOE,
                List.of("§7Compact Machine Pistol",
                        "§7High ROF | Damage: §c7",
                        "§7Short Range SMG | Range: §b~35",
                        "§8Right-click: Fire (hold)  |  Shift Drop: Reload"),
                Collections.emptyMap(), "pp2000");
    }

    private JavaPlugin pl()               { return JavaPlugin.getProvidingPlugin(PP2000.class); }
    ...
    private void bar(Player p, ItemStack stack) {
        ...
        String s = "§7" + "█".repeat(filled) + "§8" + "█".repeat(10 - filled) + " §f" + a + "§7/" + MAG_SIZE;
    ...
    private void reload(Player p, ItemStack stack) {
        ...
        if (getAmmo(stack) == MAG_SIZE) { p.sendMessage("§7Magazine already full!"); return; }
        reloading.add(p.getUniqueId());
        loopRunning.remove(p.getUniqueId());
        p.sendMessage("§7Reloading... §8(1.8s)");
        p.getWorld().playSound(p.getLocation(), Sound.BLOCK_IRON_DOOR_OPEN, 0.8f, 2.0f);
        new BukkitRunnable() { @Override public void run() {
            ...
            if (isThisItem(cur)) {
                ...
                p.getWorld().playSound(p.getLocation(), Sound.BLOCK_IRON_DOOR_CLOSE, 0.8f, 2.0f); bar(p, cur);
            } else {
    ...
    private void tryFire(Player p, ItemStack stack) {
        ...
        if (getAmmo(stack) <= 0) {
            p.sendMessage("§c*click* §7Empty! §7(Shift Drop to reload)");
    ...
    private void doShot(Player p, ItemStack stack) {
        ...
        for (double i = 0.5; i < RANGE; i += 0.4) {
            ...
            for (Entity e : p.getWorld().getNearbyEntities(loc, HIT_RADIUS, HIT_RADIUS, HIT_RADIUS)) {
                if (e instanceof LivingEntity t && e != p) {
                    double falloff = 1.0 - (i / RANGE) * 0.5;
                    double damage = DAMAGE * falloff;
                    if (Math.abs(loc.getY() - t.getEyeLocation().getY()) < 0.45) damage *= 1.6;
                    ...
                    loc.getWorld().spawnParticle(Particle.BLOCK, loc, 8, 0.1, 0.1, 0.1, blood);
                    hit = true; break;
                }
            }
            if (hit) break;
        ...
        p.getWorld().playSound(p.getLocation(), Sound.ENTITY_FIREWORK_ROCKET_BLAST, 0.5f, 2.0f); bar(p, stack);
