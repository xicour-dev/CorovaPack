...
public class M249 extends CorovaItems implements Listener {
    ...
    public M249() {
        super("§6M249", Material.DIAMOND_SWORD,
                List.of("§7Full-Auto Light Machine Gun",
                        "§7Damage: §c10 §7| Belt: §e100 §7| Reload: §c6.0s",
                        "§7Fire Rate: §d~600 RPM §7| Range: §b60",
                        "§7⚠ §eSlow reload — don't empty the belt!",
                        "§8Right-click: Fire (hold)  |  Shift Drop: Reload"),
                Collections.emptyMap(), "m249");
    }

    private JavaPlugin pl()               { return JavaPlugin.getProvidingPlugin(M249.class); }
    ...
    private void reload(Player p, ItemStack stack) {
        ...
        if (getAmmo(stack) == MAG_SIZE) { p.sendMessage("§eBelt already full!"); return; }
        reloading.add(p.getUniqueId());
        loopRunning.remove(p.getUniqueId());
        lastClickTick.remove(p.getUniqueId());
        p.sendMessage("§eReloading belt... §7(6.0s)");
    ...
    private void doShot(Player p, ItemStack stack) {
        ...
        for (double i = 0.5; i < RANGE; i += 0.35) {
            ...
            if (i % 0.7 < 0.35) p.getWorld().spawnParticle(Particle.SMOKE, loc, 1, 0, 0, 0, 0);
            for (Entity e : p.getWorld().getNearbyEntities(loc, HIT_RADIUS, HIT_RADIUS, HIT_RADIUS)) {
                if (e instanceof LivingEntity t && e != p) {
                    ...
                    loc.getWorld().spawnParticle(Particle.CRIT, loc, 6, 0.1, 0.1, 0.1, 0.03);
                    hit = true; break;
                }
            }
            if (hit) break;
        ...
        p.getWorld().playSound(p.getLocation(), Sound.ENTITY_GENERIC_EXPLODE, 1f, 1.2f); bar(p, stack);
