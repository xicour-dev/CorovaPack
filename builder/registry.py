...
class VanillaGlowAsset:
    ...
    freeze_texture_path: Path | None = None
    napalm_textures: dict[str, Path] = field(default_factory=dict)
...
FREEZE_TEXTURE_FILENAME = "freeze.png"
...
class RegistryScanner:
    ...
    def _scan_vanilla_glow(self) -> dict[str, VanillaGlowAsset]:
        ...
        for entry in sorted(folder.iterdir()):
            ...
            freeze_path = entry / FREEZE_TEXTURE_FILENAME
            freeze_texture_path = freeze_path if freeze_path.is_file() else None
            ...
            napalm_textures = {}
            for sub_entry in entry.iterdir():
                if sub_entry.is_file() and sub_entry.name.startswith("napalm") and sub_entry.suffix.lower() == ".png":
                    if sub_entry.name == "napalm.png":
                        suffix = "base"
                    elif sub_entry.name == "napalm_standby.png":
                        suffix = "standby"
                    else:
                        suffix = sub_entry.stem[7:]  # strip 'napalm_'
                    napalm_textures[suffix] = sub_entry
            ...
            assets[base_item] = VanillaGlowAsset(
                ...
                freeze_texture_path=freeze_texture_path,
                napalm_textures=napalm_textures,
            )
        return assets
