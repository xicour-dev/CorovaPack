    ...
    def compile(self, registry: Registry) -> int:
        ...
        for vanilla_glow in registry.vanilla_glow.values():
            ...
            for suffix, path in vanilla_glow.napalm_textures.items():
                if suffix == "base":
                    extra_dest = self.paths.textures_dir / "item" / "vanilla" / f"{vanilla_glow.name}_napalm.png"
                elif suffix == "standby":
                    extra_dest = self.paths.textures_dir / "item" / "vanilla" / f"{vanilla_glow.name}_napalm_standby.png"
                else:
                    extra_dest = self.paths.textures_dir / "item" / "vanilla" / f"{vanilla_glow.name}_napalm_{suffix}.png"
                util.copy_file(path, extra_dest)
                count += 1
