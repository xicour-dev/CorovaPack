...
class ModelGenerator:
    ...
    def generate(self, registry: Registry) -> int:
        ...
        for item in registry.items:
            # Check for custom model.json in the item's source directory
            custom_model_path = item.texture_path.parent / "model.json"
            if custom_model_path.is_file():
                custom_model_json = util.read_json(custom_model_path)
        ...
        for vanilla_glow in registry.vanilla_glow.values():
            ...
            # Suffix models for napalm bow/crossbow, etc.
            for suffix, path in vanilla_glow.napalm_textures.items():
                if suffix == "base":
                    parent_model = f"minecraft:item/{vanilla_glow.name}"
                    model_name = f"{vanilla_glow.name}_napalm"
                    tex_id = f"corova:item/vanilla/{vanilla_glow.name}_napalm"
                elif suffix == "standby":
                    parent_model = f"minecraft:item/{vanilla_glow.name}"
                    model_name = f"{vanilla_glow.name}_napalm"
                    tex_id = f"corova:item/vanilla/{vanilla_glow.name}_napalm_standby"
                else:
                    parent_model = f"minecraft:item/{vanilla_glow.name}_{suffix}"
                    model_name = f"{vanilla_glow.name}_napalm_{suffix}"
                    tex_id = f"corova:item/vanilla/{vanilla_glow.name}_napalm_{suffix}"
                ...
                napalm_dest = self.paths.models_dir / "vanilla" / f"{model_name}.json"
                util.write_json(napalm_dest, napalm_model_json)
                count += 1
