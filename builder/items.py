...
def _trident_fallback_model(vanilla_glow: VanillaGlowAsset) -> dict:
    flat_composite = {
        ...
        "models": [
            ...
            *_mutation_overlay_entries(vanilla_glow.glow_model_id)
        ]
    }
    return {
        "type": "minecraft:select",
...
def _spear_fallback_model(base_item: str, vanilla_glow: VanillaGlowAsset | None) -> dict:
    ...
    # flat model for inventory, ground, fixed
    flat_model_id = f"minecraft:item/{vanilla_name}"
    ...
    # in_hand model
    in_hand_model_id = f"minecraft:item/{vanilla_name}_in_hand"
    if vanilla_glow is not None and "in_hand" in vanilla_glow.extra_glows:
        in_hand_composite = {
            "type": "minecraft:composite",
            "models": [
                {
                    "type": "minecraft:model",
                    "model": in_hand_model_id
                },
                *_mutation_overlay_entries(vanilla_glow.extra_glow_model_id("in_hand"))
            ]
        }
    else:
        in_hand_composite = {
