...
def _bow_composite(vanilla_glow: VanillaGlowAsset, suffix: str | None, is_napalm: bool = False) -> dict:
    if is_napalm:
        base_model_name = f"corova:item/vanilla/bow_napalm_{suffix}" if suffix else "corova:item/vanilla/bow_napalm"
    else:
...
def _bow_fallback_model(vanilla_glow: VanillaGlowAsset, is_napalm: bool = False) -> dict:
    return {
        "type": "minecraft:condition",
        ...
        "on_true": {
            ...
            "entries": [
                {
                    "threshold": 0.65,
                    "model": _bow_composite(vanilla_glow, "pulling_1", is_napalm)
                },
                {
                    "threshold": 0.9,
                    "model": _bow_composite(vanilla_glow, "pulling_2", is_napalm)
                }
            ],
            "fallback": _bow_composite(vanilla_glow, "pulling_0", is_napalm)
        },
        "on_false": _bow_composite(vanilla_glow, None, is_napalm)
    }
...
def _crossbow_composite(vanilla_glow: VanillaGlowAsset, suffix: str | None, is_napalm: bool = False) -> dict:
    if is_napalm:
        base_model_name = f"corova:item/vanilla/crossbow_napalm_{suffix}" if suffix else "corova:item/vanilla/crossbow_napalm"
    else:
...
def _crossbow_fallback_model(vanilla_glow: VanillaGlowAsset, is_napalm: bool = False) -> dict:
    return {
        ...
        "cases": [
            {
                "when": "arrow",
                "model": _crossbow_composite(vanilla_glow, "arrow", is_napalm)
            },
            {
                "when": "rocket",
                "model": _crossbow_composite(vanilla_glow, "firework", is_napalm)
            }
        ],
        "fallback": {
            "type": "minecraft:condition",
            ...
            "on_true": {
                ...
                "entries": [
                    {
                        "threshold": 0.58,
                        "model": _crossbow_composite(vanilla_glow, "pulling_1", is_napalm)
                    },
                    {
                        "threshold": 1.0,
                        "model": _crossbow_composite(vanilla_glow, "pulling_2", is_napalm)
                    }
                ],
                "fallback": _crossbow_composite(vanilla_glow, "pulling_0", is_napalm)
            },
            "on_false": _crossbow_composite(vanilla_glow, None, is_napalm)
        }
    }
...
def _fallback_for(base_item: str, registry: Registry) -> dict:
    ...
    if vanilla_glow is not None:
        if base_item == "minecraft:bow":
            normal_fallback = _bow_fallback_model(vanilla_glow, is_napalm=False)
            napalm_fallback = _bow_fallback_model(vanilla_glow, is_napalm=True)
            return {
                "type": "minecraft:select",
                "property": "minecraft:custom_model_data",
                "index": 0,
                "cases": [
                    {
                        "when": "corova:vanilla/bow_napalm",
                        "model": napalm_fallback
                    }
                ],
                "fallback": normal_fallback
            }
        elif base_item == "minecraft:crossbow":
            normal_fallback = _crossbow_fallback_model(vanilla_glow, is_napalm=False)
            napalm_fallback = _crossbow_fallback_model(vanilla_glow, is_napalm=True)
            return {
                "type": "minecraft:select",
                "property": "minecraft:custom_model_data",
                "index": 0,
                "cases": [
                    {
                        "when": "corova:vanilla/crossbow_napalm",
                        "model": napalm_fallback
                    }
                ],
                "fallback": normal_fallback
