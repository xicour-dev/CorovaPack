...
/**
 * MP5 — Full-Auto SMG
 * Damage: 6 | Mag: 30 | Reload: 2.2s | ~750 RPM | Range: 35
 * Right-click: Fire (hold) | Shift Drop: Reload
 */
public class MP5 extends CorovaItems implements Listener {

    private static final double DAMAGE     = 2.75;
    ...
    private static final long   RELOAD_MS  = 2200L;
    ...
    public MP5() {
        super("§bMP5", Material.IRON_SWORD,
                List.of("§7Rapid-fire SMG",
                        "§7Damage: §c5.5 §7| Mag: §e30 §7| Reload: §a2.2s",
                        "§7Fire Rate: §d~750 RPM §7| Range: §b35",
                        "§8Right-click: Fire (hold)  |  Shift Drop: Reload"),
                Collections.emptyMap(), "mp5");
    }

    private JavaPlugin pl()               { return JavaPlugin.getProvidingPlugin(MP5.class); }
    ...
    private void reload(Player p, ItemStack stack) {
        ...
        p.sendMessage("§eReloading... §7(2.2s)");
        ...
        for (double i = 0.5; i < RANGE; i += 0.35) {
            ...
            for (Entity e : p.getWorld().getNearbyEntities(loc, HIT_RADIUS, HIT_RADIUS, HIT_RADIUS)) {
                if (e instanceof LivingEntity t && e != p) {
                    ...
                    loc.getWorld().spawnParticle(Particle.CRIT, loc, 5, 0.1, 0.1, 0.1, 0.02);
                    hit = true; break;
                }
            }
            if (hit) break;
