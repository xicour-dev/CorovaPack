...
public class WitherBook extends EnchantmentBook {
    ...
    public WitherBook(int level) {
        super(
                "Book of Wither",
                CorovaEnchantments.WITHER_ID,
                level,
                "book_wither",
                allowedMaterials()
        );
    ...
    public static void triggerEffect(LivingEntity damager, LivingEntity target, int level) {
        ...
        // Apply wither effect (duration: 8 seconds, level matches enchant level)
        // Wither effect levels are 0-indexed, so we subtract 1
        target.addPotionEffect(new PotionEffect(
                PotionEffectType.WITHER,
                8 * 20, // 8 seconds
                level - 1 // Effect amplifier (0 = Wither I)
        ));

        ItemStack weapon = (damager.getEquipment() != null) ? damager.getEquipment().getItemInMainHand() : null;
        MutationManager.getInstance().getSynergyHandler().handleEnchantSynergy(CorovaEnchantments.WITHER_ID, damager, target, weapon, level);

        // Black particle effect
        Particle.DustOptions witherDust = new Particle.DustOptions(Color.fromRGB(60, 60, 60), 1.5f);
    ...
    public void onEntityDamage(EntityDamageByEntityEvent event) {
        ...
        // Check if weapon has wither enchantment
        if (!CorovaEnchantments.hasEnchant(weapon, getEnchantId())) return;

        // Get the wither level from the weapon
        int witherLevel = CorovaEnchantments.getEnchantLevel(weapon);
        if (witherLevel <= 0) return;

        triggerEffect(damager, target, witherLevel);
    }
    ...
    public static ItemStack getUpgradedBook(ItemStack book1, ItemStack book2, org.bukkit.NamespacedKey keyId, org.bukkit.NamespacedKey keyLvl) {
        ...
        if (id1 == null || !id1.equals(id2) || !id1.equals(CorovaEnchantments.WITHER_ID)) {
            return null;
        }
        ...
        WitherBook upgradedBook = new WitherBook(newLevel);
